...
import co.com.personal.hnino.proyectoapolo.data.network.ImgsApolloAPIClientInteface
...
// Este modulo, nos va a proveer dependencias (necesario para usar inyeción de dependencias con Dagger Hiil) que no son faciles de proveer, lo
...
// y/o habilitar para ser inyectada  en otras clases y para ello usamos @Provides
...
object NetworkModuleProvider { // Los objetos pueden ser accedidos de manera directa, como su fueran variables o funciones dentro de un
    ...
    fun provideImgsApoloRetrofit(): Retrofit { // retorna un objeto de tipo Retrofit. Como esta función esta configurada como @Provides, significa
        ...
        // que seria la clase ImgApolo etc
    ...
    @Singleton // Usamos el patron de diseño Singleton para mantener una unica instancia de la interfaz ImgsApolloAPIClientInteface, de tal manera que
    // no se creen varias instancias cada vez que llamemos a la interfaz ImgsApolloAPIClientInteface y con esto evitar consumir recursos de memoria sin tener la necesitad.
    @Provides // Con esta etiqueta habilitamos este modulo para que => si cualquiera de nuestras clases necesita una instancia de la interfaz ImgsApolloAPIClientInteface,
    // Dagger Hilt va a saber como darsela, es decir se va a poder inyectarsela en la clase que necesite esta dependencia.
    fun provideImgsApoloApiClient(retrofit: Retrofit): ImgsApolloAPIClientInteface { // Esta funcion retorna un objeto de tipo ImgsApolloAPIClientInteface. Como esta función
        // esta configurada como @Provides, significa que si en cualquier clase necesitamos una instacia de interfaz ImgsApolloAPIClientInteface, simplemente la
        // inyectamos, y Dagger Hilt va a buscar y a encontrar a esta funcion, que retorna un objeto de ImgsApolloAPIClientInteface y la va a proveer.

        // (retrofit: Retrofit) es proveido por la función provideImgsApoloRetrofit existente en este mismo modulo. Esto Funciona
        // debido a que la función provideImgsApoloRetrofit es una función @Provides y esa nos provee retrofit para inyectarla en cualquier
        // clase que la requiera y tambien se puede proveer en otras funciones @Provides como sucede en este caso

        return retrofit.create(ImgsApolloAPIClientInteface::class.java) // Se retorna un objeto de tipo ImgsApolloAPIClientInteface.
